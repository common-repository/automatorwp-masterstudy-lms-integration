...
final class AutomatorWP_MasterStudy_LMS_Integration {
    ...
    private function constants() {
        // Plugin version
        define( 'AUTOMATORWP_MASTERSTUDY_LMS_VER', '1.0.2' );
    ...
    public function admin_notices() {
        ...
        if ( ! $this->meets_requirements() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php printf(
                        __( 'AutomatorWP - MasterStudy LMS requires %s and %s in order to work. Please install and activate them.', 'automatorwp-masterstudy-lms-integration' ),
        ...
        <?php elseif ( $this->pro_installed() && ! defined( 'AUTOMATORWP_ADMIN_NOTICES' ) ) : ?>
            ...
            <div id="message" class="notice notice-error is-dismissible">
                <p>
                    <?php echo __( 'You can uninstall AutomatorWP - MasterStudy LMS Integration because you already have the pro version installed and includes all the features of the free version.', 'automatorwp-masterstudy-lms-integration' ); ?>
                </p>
            </div>
    ...
    public function load_textdomain() {
        ...
        $lang_dir = AUTOMATORWP_MASTERSTUDY_LMS_DIR . '/languages/';
        $lang_dir = apply_filters( 'automatorwp_masterstudy_lms_languages_directory', $lang_dir );

        // Traditional WordPress plugin locale filter
        $locale = apply_filters( 'plugin_locale', get_locale(), 'automatorwp-masterstudy-lms-integration' );
        $mofile = sprintf( '%1$s-%2$s.mo', 'automatorwp-masterstudy-lms-integration', $locale );

        // Setup paths to current locale file
        $mofile_local   = $lang_dir . $mofile;
        $mofile_global  = WP_LANG_DIR . '/automatorwp-masterstudy-lms-integration/' . $mofile;

        if( file_exists( $mofile_global ) ) {
            // Look in global /wp-content/languages/automatorwp-masterstudy-lms-integration/ folder
            load_textdomain( 'automatorwp-masterstudy-lms-integration', $mofile_global );
        } elseif( file_exists( $mofile_local ) ) {
            // Look in local /wp-content/plugins/automatorwp-masterstudy-lms-integration/languages/ folder
            load_textdomain( 'automatorwp-masterstudy-lms-integration', $mofile_local );
        } else {
            // Load the default language files
            load_plugin_textdomain( 'automatorwp-masterstudy-lms-integration', false, $lang_dir );
        }
